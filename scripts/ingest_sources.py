...
def load_yaml(yaml_path: Path) -> dict[str, Any]:
    ...
    if not isinstance(config, dict):
        raise ValueError("source configuration must be a YAML mapping")
    return config
...
def ingest_source(yaml_path: Path, ingestion_run_id: str) -> str:
    ...
    if original_pdf_path.is_file():
        stored_hash = calculate_sha256(original_pdf_path)
    ...
    copied_hash = calculate_sha256(temporary_pdf_path)
